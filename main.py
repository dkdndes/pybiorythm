...
Examples:
  ...
  python main.py -y 1990 -m 5 -d 15 --days 30 --orientation json-vertical
    ...
    parser.add_argument('-y', '--year', type=int, 
                       help='Birth year (1-9999)')
    parser.add_argument('-m', '--month', type=int,
                       help='Birth month (1-12)')  
    parser.add_argument('-d', '--day', type=int,
                       help='Birth day (1-31)')
    parser.add_argument('--orientation', choices=['vertical', 'horizontal', 'json-vertical', 'json-horizontal'],
                       default='vertical',
                       help='Chart orientation (default: vertical)')
    parser.add_argument('--days', type=int, default=29,
                       help='Number of days to plot (default: 29)')
...
def main():
    ...
    if not any([args.year, args.month, args.day]):
        ...
        if not all([args.year, args.month, args.day]):
            print("Error: When using command line arguments, all date components (--year, --month, --day) must be provided")
            sys.exit(1)
