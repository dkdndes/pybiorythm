...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--name", type=str, required=True, help="Name of the person"
        )
        parser.add_argument(
            "--birthdate",
            type=str,
        ...
        parser.add_argument(
            "--email", type=str, default=None, help="Optional email address"
        )
        parser.add_argument(
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                biorhythm_json = calc.generate_timeseries_json(
                    birthdate_dt, target_date_dt
                )

                # Create calculation record
                start_date = datetime.strptime(
                    biorhythm_json["data"][0]["date"], "%Y-%m-%d"
                ).date()
                end_date = datetime.strptime(
                    biorhythm_json["data"][-1]["date"], "%Y-%m-%d"
                ).date()
                ...
                calculation = BiorhythmCalculation.objects.create(
                    ...
                    pybiorythm_version=biorhythm_json.get("meta", {}).get(
                        "version", "unknown"
                    ),
                ...
                if options["force"]:
                    deleted_count = BiorhythmData.objects.filter(
                        person=person
                    ).delete()[0]
                    if deleted_count > 0:
                        self.stdout.write(
                            f"🗑️  Deleted {deleted_count} existing data points"
                        )
