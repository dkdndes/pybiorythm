...
def dashboard_home(request: HttpRequest) -> HttpResponse:
    ...
    if api_info:
        ...
        context["error_message"] = (
            "Unable to connect to PyBiorythm API. Please check the server is running."
        )
    ...
    try:
        ...
        if biorhythm_response and "biorhythm_data" in biorhythm_response:
            chart_data = create_biorhythm_line_chart(
                biorhythm_response["biorhythm_data"], person_data["name"]
            )
        else:
            chart_data = create_empty_chart("No biorhythm data available")

        return JsonResponse({"chart_data": chart_data, "config": get_chart_config()})
...
def chart_critical_days(request: HttpRequest, person_id: int) -> JsonResponse:
    ...
    try:
        ...
        logger.error(
            f"Error generating critical days chart for person {person_id}: {e}"
        )
        return JsonResponse(
            {
                "chart_data": create_empty_chart("Error loading chart data"),
...
def chart_correlation(request: HttpRequest, person_id: int) -> JsonResponse:
    ...
    try:
        ...
        biorhythm_response = api_client.get_biorhythm_data_fresh(person_id, limit=5000)
        logger.info(
            f"Biorhythm response for person {person_id}: {biorhythm_response is not None}"
        )

        if biorhythm_response and "biorhythm_data" in biorhythm_response:
            biorhythm_data = biorhythm_response["biorhythm_data"]
            logger.info(
                f"Got {len(biorhythm_data)} biorhythm data points for correlation"
            )

            chart_data = create_correlation_chart(biorhythm_data, person_data["name"])
...
def chart_statistics(request: HttpRequest, person_id: int) -> JsonResponse:
    ...
    try:
        ...
        if statistics:
            chart_data = create_statistics_summary_chart(
                statistics, person_data["name"]
            )
        else:
...
def calculate_biorhythm(request: HttpRequest, person_id: int) -> HttpResponse:
    ...
    try:
        ...
        if result:
            messages.success(
                request, f"Successfully calculated {days} days of biorhythm data."
            )
            return render(
                request,
                "dashboard/calculation_success.html",
                {
            ...
            return render(
                request,
                "dashboard/calculation_error.html",
        ...
        return render(request, "dashboard/calculation_error.html", {"error": str(e)})
...
def people_list_partial(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        request,
        "dashboard/partials/people_list.html",
        {"people": people, "search_query": search},
    )
...
def person_info_partial(request: HttpRequest, person_id: int) -> HttpResponse:
    ...
    return render(
        request,
        "dashboard/partials/person_info.html",
        {"person": person_data, "statistics": statistics},
    )
            ...
            return JsonResponse(
                {
                    ...
                    "physical_range": [
                        float(df["physical"].min()),
                        float(df["physical"].max()),
                    ],
                    "emotional_range": [
                        float(df["emotional"].min()),
                        float(df["emotional"].max()),
                    ],
                    "intellectual_range": [
                        float(df["intellectual"].min()),
                        float(df["intellectual"].max()),
                    ],
                    "correlation_matrix_dict": {
                        "physical_emotional": float(
                            correlation_matrix.loc["physical", "emotional"]
                        ),
                        "physical_intellectual": float(
                            correlation_matrix.loc["physical", "intellectual"]
                        ),
                        "emotional_intellectual": float(
                            correlation_matrix.loc["emotional", "intellectual"]
                        ),
                    },
                }
            )
...
def test_exact_correlation(request: HttpRequest) -> JsonResponse:
    ...
    correlation_data = [[1.0, 0.238, -0.048], [0.238, 1.0, 0.380], [-0.048, 0.380, 1.0]]
    ...
    fig.update_layout(title="Test Exact Correlation Values", height=400, width=500)
