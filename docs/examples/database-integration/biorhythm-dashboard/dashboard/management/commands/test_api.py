...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"API Token: {'Set' if settings.PYBIORYTHM_API_TOKEN else 'Not set'}"
        )
        self.stdout.write("")
        ...
        self.stdout.write(
            f"   PyBiorythm Available: {api_info.get('pybiorythm_available', False)}"
        )
        self.stdout.write("")
        ...
        if people_data:
            ...
            self.stdout.write(
                self.style.SUCCESS(
                    f"✅ People endpoint working - {people_count} people found"
                )
            )
        ...
        if stats:
            ...
            self.stdout.write(
                f"   Total data points: {stats.get('total_data_points', 0)}"
            )
            self.stdout.write(
                f"   Critical days: {stats.get('total_critical_days', 0)}"
            )
        else:
            self.stdout.write(
                self.style.WARNING("⚠️  Statistics endpoint returned no data")
            )
