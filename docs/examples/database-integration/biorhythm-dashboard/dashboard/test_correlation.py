...
def test_simple_heatmap():
    ...
    correlation_data = [[1.0, 0.1, -0.2], [0.1, 1.0, 0.3], [-0.2, 0.3, 1.0]]
    ...
    fig.update_layout(title="Test Correlation Heatmap", height=400, width=400)
...
def test_real_correlation_heatmap():
    ...
    fig.add_trace(
        go.Heatmap(
            ...
            showscale=True,
            colorbar=dict(
                title="Correlation Coefficient", tickmode="linear", tick0=-1, dtick=0.5
            ),
        )
    )
