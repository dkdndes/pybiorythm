...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default='',
            help='Optional notes about this person'
        )
        parser.add_argument(
            '--batch-size',
            type=int,
        ...
        parser.add_argument(
            '--force',
            action='store_true',
            help='Force overwrite if person already exists'
        )
    ...
    def handle(self, *args, **options):
        ...
        if options['target_date']:
            try:
                ...
                raise CommandError(f"Invalid target date format: {options['target_date']}. Use YYYY-MM-DD")
        else:
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(f"📊 Processing {total_points} data points in batches of {batch_size}...")
                ...
                for i, day_data in enumerate(biorhythm_json['data']):
                    ...
                    if len(data_points) >= batch_size:
                        ...
                        self.stdout.write(f"💾 Saved batch: {i+1-len(data_points)+1}-{i+1} of {total_points}")
                        data_points = []

                # Insert remaining data points
                if data_points:
                    BiorhythmData.objects.bulk_create(data_points)
                    self.stdout.write(f"💾 Saved final batch: {total_points-len(data_points)+1}-{total_points}")

                # Summary
                total_saved = BiorhythmData.objects.filter(person=person).count()
                critical_days = BiorhythmData.objects.filter(
                    person=person
                ).filter(
