        ...
        def generate_timeseries_json(self, birthdate, start_date=None):
            ...
            for i in range(self.days):
                ...
                if abs(physical) < 0.05:
                    critical_days.append("physical_" + ("positive" if physical >= 0 else "negative"))
                if abs(emotional) < 0.05:
                    critical_days.append("emotional_" + ("positive" if emotional >= 0 else "negative"))  
                if abs(intellectual) < 0.05:
                    critical_days.append("intellectual_" + ("positive" if intellectual >= 0 else "negative"))
...
def main():
    parser = argparse.ArgumentParser(description='Generate sample biorhythm datasets for analysis')
